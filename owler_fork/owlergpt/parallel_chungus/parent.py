...
selected_models = [
    m for m in models if excluded_model_substring.lower() not in m.lower()
]
...
device_list = [
    "cuda:0"
]  # you can have multiple devices if you want parallelization across GPUs
        ...
        for model_name in selected_models:
            for device_id, device in enumerate(device_list):
                ...
                if result.returncode != 0:
                    print(
                        f"Error embedding dataset={dataset} with model={model_name} on device={device}. Return code: {result.returncode}"
                    )
                else:
                    print(
                        f"Successfully embedded dataset={dataset} with model={model_name} on device={device}"
                    )
