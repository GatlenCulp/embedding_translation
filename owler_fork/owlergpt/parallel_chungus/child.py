...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-memory", required=True, help="e.g. 14GB. Not strictly used by code, but can be enforced by CUDA_VISIBLE_DEVICES if needed.")
    ...
    for jsonl_file in jsonl_files:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            assert batch_emb.shape[1] == dim, f"Embedding dimension mismatch. Got {batch_emb.shape[1]}, expected {dim}"
        ...
        if len(all_embeddings) > 0:
            ...
            # No chunks
            all_embeddings_pt = torch.empty((0, model2model_dimension(model_name)), dtype=torch.float32)
        ...
        safetensors.torch.save_file({"embeddings": all_embeddings_pt}, sub_dir / "embeddings.safetensors")

        # Copy metadatas (just the same jsonl)
        with jsonl_file.open("r") as f_in, (sub_dir / "metadatas.jsonl").open("w") as f_out:
            for line in f_in:
                f_out.write(line)
