...
class EmbeddingMetadataPopulatorCoordinator:
    ...
    """
    def __init__(self, selected_folder: str, collections: List[Collection], metadata_populator_args: EmbeddingMetadataPopulatorArgs):
    ...
    def get_embedding_metadata(self, chunk_metadata: str) -> List[EmbeddingMetadata]:
        ...
        chunk_text = chunk_metadata["record_text"] # NOTE by this they SEEM to actually mean chunk text???
        record_id = chunk_metadata["record_id"]
        if self.already_populated_one_collection:
            assert chunk_text in self.chunk_text2embedding_metadata, f"Chunk text not found in metadata: {chunk_text}"
            return self.chunk_text2embedding_metadata[chunk_text]
        else:
            assert not chunk_text in self.chunk_text2embedding_metadata, f"Chunk text should not be in metadata: {chunk_text}"
            tags = {}
            return [
                EmbeddingMetadata(
                    chunk_id=str(uuid4()),
    ...
    def validate_collections(self):
        ...
        assert all(size == sizes[0] for size in sizes), f"All collections must have the same number of elements: {sizes}"
        # 2. validate that either all or none have been populated (collection granularity)
        metadata_populated = [self.__collection_metadata_is_populated(collection) for collection in self.collections]
        assert all(metadata_populated) or not any(metadata_populated), f"Either all or none collections must have been populated: {metadata_populated}"
        # 3. validate that the train-test split is consistent across collections
        all_is_popped: List[bool] = []
        for collection in self.collections:
            is_popped: bool = any(self.__entry_metadata_is_populated(metadatas) for _, metadatas in collection.get()["metadatas"])
            all_is_popped.append(is_popped)
        all_is_popped_dump = '\n'.join([f"{collection.name}: {is_popped}" for collection, is_popped in zip(self.collections, all_is_popped)])
        assert all(all_is_popped) or not any(all_is_popped), f"Either all or none collections must have been populated: {all_is_popped_dump}"
...
class EmbeddingMetadataPopulator:
    ...
    def infer_ingestion_settings(self) -> IngestionSettings:
        ...
        if "hnsw:space" in self.collection.metadata:
            assert distance_function == self.collection.metadata["hnsw:space"], f"Distance function mismatch: {distance_function} != {self.collection.metadata['hnsw:space']} (disable environ?)"
            distance_function = self.collection.metadata["hnsw:space"]
        assert device not in self.collection.metadata, f"Device should not be in metadata: {self.collection.metadata} (reserved name, but not used rn)"
        if "normalize_embeddings" in self.collection.metadata:
            normalize_embeddings = self.collection.metadata["normalize_embeddings"]
    ...
    def infer_embedding_dataset_information(self) -> EmbeddingDatasetInformation:
        ...
        try:
            ...
            if not self.metadata_populator_args.overwrite_metadata:
                raise EmbeddingMetadataPopulatorIdempotencyBug("Embedding collection metadata already properly setup")
            return obj
        except ValidationError as e:
            inferred_ingestion = self.infer_ingestion_settings()
            embedding_model_name, text_dataset, chromadb_collection_name = parse_collection_name(self.collection.name)
