...
class SingletonInjestor:
    ...
    def get_jsonl_file_doctype(self, jsonl_file: Path) -> str:
        if "corpus" in jsonl_file.name and not ("query" in jsonl_file.name or "queries" in jsonl_file.name):
            return "corpus"
        elif ("query" in jsonl_file.name or "queries" in jsonl_file.name) and not "corpus" in jsonl_file.name:
            return "queries"
        else:
            raise ValueError(f"Unknown jsonl file type: {jsonl_file}")
    def get_jsonl_file_traintype(self, jsonl_file: Path) -> str:
        if "train" in jsonl_file.name and not "validation" in jsonl_file.name:
            return "train"
        elif "validation" in jsonl_file.name and not "train" in jsonl_file.name:
            return "validation"
        else:
            raise ValueError(f"Unknown jsonl file type: {jsonl_file}")

    def get_embeddings_metadatas_target_dir_paths(self, jsonl_file: Path, transformer_name: str) -> Tuple[Path, Path, Path]:
        ...
        assert jsonl_file.name == f"{jsonl_doctype}_{jsonl_traintype}.jsonl", f"Expected {jsonl_file.name} to be {jsonl_doctype}.{jsonl_traintype}.jsonl"
        # 2. Dataset
        assert self.dataset_chosen in DATASETS, f"Expected {self.dataset_chosen} to be a dataset, but not in {DATASETS}"
        assert jsonl_file.parent.name == self.dataset_chosen, f"Expected {jsonl_file.parent.name} to be {self.dataset_chosen}" # one dataset per :P
        ...
        embeddings_file_path = target_path / f"embeddings_{jsonl_doctype}_{jsonl_traintype}.safetensors"
        metadatas_file_path = target_path / f"metadatas_{jsonl_doctype}_{jsonl_traintype}.jsonl"
    ...
    def ingest(self) -> None:
        ...
        jsonl_files_list = [x for x in jsonl_files_list if x.name not in ["corpus.jsonl", "queries.jsonl"]] # we don't do these again (they are in train/validation)
        assert all(x.name in ["corpus_train.jsonl", "corpus_validation.jsonl", "queries_train.jsonl", "queries_validation.jsonl"] for x in jsonl_files_list)
        ...
        assert all((parent / "queries_validation.jsonl").exists() for parent in _parents)
        print("[END SANS]")
        for transformer_names in self.transformer_names:
            ...
            transformer_names_folderable = [t.replace("/", "_") for t in transformer_names]
            assert len(set(transformer_names_folderable)) == len(transformer_names)
            print("---------------------------------------- PROCESSING JSONL FILES ----------------------------------------")
            # TODO(Adriano) parallel plz
            for transformer, transformer_name_folderable, transformer_name in zip(transformers, transformer_names_folderable, transformer_names):
                for jsonl_file in tqdm(jsonl_files_list, desc="Processing jsonl files"):
                    embeddings_file_path, metadatas_file_path, target_dir = self.get_embeddings_metadatas_target_dir_paths(jsonl_file, transformer_name_folderable)
                    target_dir.mkdir(parents=True, exist_ok=True)
                    assert not embeddings_file_path.exists() # every write should be new!
                    ...
                    # 1. Save embeddings
                    safetensors.torch.save_file({"embeddings": all_embeddings_pt}, embeddings_file_path)
...
def ingest_hf(output: str, device: str, parallelize: bool, dataset: str):
    ...
    injestor = SingletonInjestor(Path(output), device, parallelize, dataset) # <--- no side-effects
