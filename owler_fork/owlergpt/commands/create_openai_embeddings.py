...
@click.option(
    ...
    type=str,
    default=None,
    help="Pass the chunks path to be able to just load chunks and work on those instead.",
)
def ingest_openai(
    dataset_path: str | None,
    output_path: str | None,
    regex_datasets: str | None,
    ...
    for dataset_folder in dataset_folders:
        ...
        assert not (output_path / dataset_folder.name).exists() or (
            len(list((output_path / dataset_folder.name).glob("*"))) == 0
        ), f"Dataset folder {dataset_folder.as_posix()} already exists"
