...
class JSONDataset(Dataset):
    ...
    def __init__(
        ...
        splitter: langchain.text_splitter,
        model_name: str,
        chunk_size: int,
        chunk_overlap: int,
        ...
        self.path = path
        self.jsonl = pd.read_json(open(path), lines=True)
    ...
    def __getitem__(self, idx: int):
        ...
        ids = [
            f"{doc_id}-{i + 1}-{self.record_type}" for i in range(len(text_chunks))
...
def collate_fn(data):
    ...
    """
    documents, ids, text_chunks = zip(*data, strict=False)
...
def _get_text_chunks(
    ...
    :param client: The Cohere client used to revert the tokens to text (or generally a splitter)
