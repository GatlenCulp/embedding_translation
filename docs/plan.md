...
1. Small T5 model
1. Another small Hugging Face model
1. GPT embeddings
1. (Optional) Medium-sized model like Llama-base
...
1. Generate embeddings on test dataset
1. Create UMAP visualization with prompts/labels
1. Generate tables comparing:
...
1. Visualization code and blog framework
1. Initial blog with placeholder data
1. Final blog with real experimental results
...
1. One dataset
1. Two models
1. Two embedding translators with four analyses (A -> B) (B -> A)
1. Kernel Tables
1. Aggregate Statistics, MSE (Across all permutations) and KNN (Did the 20%)
...
1. $C$ cross-encoders [$C = 2$]
1. $Z$ embedding models
1. $z\_\\texttt{A}$ denotes an embedding model named $\\texttt{A}$
1. $\\varepsilon\_\\texttt{A}$ denotes the true embedding space of an embedding model named $\\texttt{A}$
1. $c^{\\texttt{(architecture)}}_{\\varepsilon_\\texttt{A} \\to \\hat{\\varepsilon}_\\texttt{B}}$ denotes an embedding translator with $\\texttt{architecture}$ trained to translate embeddings such that $c^{\\texttt{(architecture)}}_{\\varepsilon\_\\texttt{A} \\to \\hat{\\varepsilon}_\\texttt{B}}(z_\\texttt{A}(x)) \\approx z\_\\texttt{B}(x)$
1. There are $C$ total embedding translators
1. $\\hat{\\varepsilon}\_\\texttt{A}$ denotes an approximate embedding space of an embedding model named $\\texttt{A}$ (there may be multiple)
1. $E$ embedding datasets [$E = X * D$]
1. $\\hat{E}_{\\varepsilon_\\texttt{A} \\to \\hat{\\varepsilon}\_\\texttt{B}}$ are translated embedding datasets. bah
1. Lets say there are $A$ architectures

\\

1. Load $D$ textual datasets (possibly HotPotQA) [$D = 1$]
1. Split each dataset $d \\in {1, \\cdots, D}$ datasets into 80% $d\_\\texttt{train}$, 20% $d\_\\texttt{test}$
1. For each of $Z$ embedding models and each of $D$ datasets, create $E = Z * D$ embedding datasets.
1. For each of $A$ architectures and $\\forall i, j \\in {1, ..., E}, i \\neq j$, train embedding translator $c^{A}\_{i \\to j}$ creating $A * E * (E - 1)$ translation embedding datasets
1. Aaaaaa
