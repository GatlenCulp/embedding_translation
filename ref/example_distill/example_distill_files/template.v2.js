((factory) => {
	 ...
	 */
	const renderMathInText = (text, optionsCopy) => {
	...
	const renderElem = (elem, optionsCopy) => {
	...
	const renderMathInElement = (elem, options) => {
		if (!elem) {
...
`,
	...
	function author_string(ent, template, sep, finalSep) {
		...
		const name_strings = names.map((name) => {
			name = name.trim();
			if (name.indexOf(",") != -1) {
...
`,
	...
	var bibtexParse = createCommonjsModule((module, exports) => {
...
`;
	...
	var prism = createCommonjsModule((module) => {
		/* **********************************************
       Begin prism-core.js
  ********************************************** */
  ...
  ********************************************** */
		...
		(() => {
			 ...
			 */
			self.Prism.fileHighlight = (container) => {
...
`,
							...
							: (m = reHslPercent.exec(format))
								...
								: (m = reHslaPercent.exec(format))
									...
									: named.hasOwnProperty(format)
										...
										: format === "transparent"
											? new Rgb(Number.NaN, Number.NaN, Number.NaN, 0)
											: null;
	}
	...
	function normalize(a, b) {
		return (b -= a = +a)
			? (x) => (x - a) / b
			: constant$1(isNaN(b) ? Number.NaN : 0.5);
	}
	...
	function transformer() {
		...
		scale.range = (_) =>
			arguments.length ? ((range = Array.from(_)), rescale()) : range.slice();
		...
		scale.interpolate = (_) =>
			arguments.length ? ((interpolate$1 = _), rescale()) : interpolate$1;

		scale.unknown = (_) =>
			arguments.length ? ((unknown = _), scale) : unknown;
	...
	function linear$1() {
		...
		scale.copy = () => copy(scale, linear$1());
