...
if (document.getElementById && document.childNodes && document.createElement) {
	if (!(window.MathJax && MathJax.Hub)) {
		...
		((d) => {
			...
			b.Object = c({
				...
				SimpleSUPER: c({
					...
					wrap: (i, h) => {
						...
						g.toString = () => h.toString.apply(h, arguments);
						return g;
					},
				}),
			});
			b.Object.isArray =
				Array.isArray ||
				((f) => Object.prototype.toString.call(f) === "[object Array]");
		...
		((BASENAME) => {
			...
			var CALLBACK = (data) => {
				var cb = () =>
					arguments.callee.execute.apply(arguments.callee, arguments);
			...
			var ISCALLBACK = (f) => typeof f === "function" && f.isCallback;
			var EVAL = (code) => eval.call(window, code);
			var TESTEVAL = () => {
			...
			var DELAY = (time, callback) => {
				callback = USING(callback);
				callback.timeout = setTimeout(callback, time);
				return callback;
			};
			var WAITFOR = (callback, signal) => {
				callback = USING(callback);
				if (!callback.called) {
			...
			var WAITSIGNAL = (callback, signals) => {
				if (!isArray(signals)) {
					signals = [signals];
				}
				if (!callback.signal) {
			...
			var EXECUTEHOOKS = (hooks, data, reset) => {
				if (!hooks) {
					return null;
				}
				if (!isArray(hooks)) {
		...
		MathJax.Message = {
			...
			localize: (a) => MathJax.Localization._(a, a),
		...
		MathJax.Hub = {
			...
			Register: {
				...
				MessageHook: () =>
					MathJax.Hub.signal.MessageHook.apply(MathJax.Hub.signal, arguments),
				StartupHook: () =>
					MathJax.Hub.Startup.signal.MessageHook.apply(
						MathJax.Hub.Startup.signal,
						arguments,
					),
				LoadHook: () => MathJax.Ajax.LoadHook.apply(MathJax.Ajax, arguments),
			},
			UnRegister: {
				PreProcessor: (a) => {
			...
			formatError: function (b, f) {
				var h = (l, k, j, i) => MathJax.Localization._(l, k, j, i);
			...
			SplitList:
				"trim" in String.prototype
					? (a) => a.trim().split(/\s+/)
					: (a) => a.replace(/^\s+/, "").replace(/\s+$/, "").split(/\s+/),
		};
		...
		MathJax.Hub.Startup = {
			...
			Config: function () {
				...
				this.queue.Push(
					...
					["ConfigBlocks", this],
					[
						(d) => d.loadArray(MathJax.Hub.config.config, "config", null, true),
						this,
					],
			...
			onLoad: function () {
				var a = (this.onload = MathJax.Callback(() => {
		...
		((d) => {
			...
			b.OutputJax = g.Subclass(
				{
					...
					preProcess: function (j) {
						...
						this.constructor.prototype.preProcess = (k) => {
							if (!i.called) {
								return i;
							}
			...
			b.OutputJax.Error = {
				...
				getJaxFromMath: (h) => (h.nextSibling.MathJax || {}).error,
				Jax: function (j, i) {
		...
		((o) => {
			...
			var a = {
				...
				versionAtLeast: function (y) {
					...
					for (var z = 0, j = y.length; z < j; z++) {
						if (x[z] != y[z]) {
							return Number.parseInt(x[z] || "0") >= Number.parseInt(y[z]);
						}
					}
					return true;
				},
			...
			try {
				d.Browser.Select({
					Safari: (j) => {
						var i = Number.parseInt(String(j.version).split(".")[0]);
						if (i > 85) {
							j.webkit = j.version;
						}
						if (i >= 538) {
