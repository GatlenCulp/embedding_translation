...
- python3.10 ./scripts/encryption.py generate-key                                               './secrets/embedding_translation.fernet.key'
- python3.10 ./scripts/encryption.py process encrypt ./secrets           ./secrets.encrypted -k './secrets/embedding_translation.fernet.key'
- python3.10 ./scripts/encryption.py process decrypt ./secrets.encrypted ./secrets.decrypted -k './secrets/embedding_translation.fernet.key'
"""
...
def get_key(key_file: str | None) -> bytes:
    ...
    if key_file:
        ...
        passphrase = click.prompt(
            "Enter the passphrase", hide_input=True, confirmation_prompt=True
        )
...
def process_file(
    ...
    try:
        ...
        raise click.ClickException(
            f"Error decrypting file {input_path}: Invalid token. Ensure you're using the correct key."
        )
...
def process_directory(
    ...
    try:
        ...
        click.echo(
            f"Successfully {'encrypted' if encrypt else 'decrypted'} {'directory' if encrypt else 'file or directory'}"
        )
    except Exception as e:
        raise click.ClickException(
            f"Error processing {'directory' if encrypt else 'file or directory'}: {e}"
        )
...
@click.option(
    "-k", "--key-file", type=click.Path(exists=True), help="Path to the key file"
)
def process(action: str, input_path: str, output_path: str, key_file: str | None):
    try:
        ...
        if input_path.is_file() and action == "encrypt":
            ...
            raise click.ClickException(
                f"Error: Invalid input/output combination for {action}"
            )
...
def generate_key(key_file: str):
    ...
    try:
        passphrase = click.prompt(
            "Enter the passphrase to generate the key",
            hide_input=True,
